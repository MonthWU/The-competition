...
- `puzzle_solver_node`
  ...
  - 发挥题一使用 90--120 mm × 50--90 mm 尺寸范围，目标方向和下半区内中心位置自由；
    ...
    选择，不沿用基础题的严格分差拒绝。
  - 发挥题二复用同一几何搜索和 A4 下半区目标边界；接缝纹理评分只用于在几何
    非唯一或近似等价时选择更好的候选，默认不要求绝对质量上限或非零分差。
  - 发挥题二检测到边界内侧紧挨花纹的高信息量边时，会优先比较这些边及其对应碎片；
    弱纹理白边仍按几何可行性参与排序。
- `puzzle_coordinator_node`
  ...
  - 发挥题二先与发挥题一保持同一闭环：收到当前片 `[move,ok]` 后重新扫描和重求解；
    扑克牌花纹只作为几何候选通过后的接缝排序依据。
- `puzzle_geometry`
