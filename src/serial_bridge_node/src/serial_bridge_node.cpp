...
geometry_msgs::msg::Point32 green_a4_normalized_point(
  const geometry_msgs::msg::Point32 & point_a4_mm)
{
  geometry_msgs::msg::Point32 normalized;
  normalized.x = static_cast<float>(point_a4_mm.x / kA4WidthMm);
  normalized.y = static_cast<float>(1.0 - point_a4_mm.y / kA4HeightMm);
  normalized.z = 0.0F;
  return normalized;
}
}  // namespace
  ...
  hmi_received_count_(0U), queue_drop_count_(0U), hmi_queue_drop_count_(0U)
{
  ...
  if (placement_coordinate_frame_ != "camera_center_px" &&
    placement_coordinate_frame_ != "a4_mm" &&
    placement_coordinate_frame_ != "green_a4_normalized" &&
    placement_coordinate_frame_ != "workspace_mm")
  {
    throw std::runtime_error(
            "placement_coordinate_frame must be camera_center_px, a4_mm, green_a4_normalized, "
            "or workspace_mm");
  }
  ...
  map_result_subscription_ = create_subscription<std_msgs::msg::String>(
    map_result_topic_, 10,
    std::bind(&SerialBridgeNode::on_map_result, this, std::placeholders::_1));
  ...
  const vision_interfaces::msg::TaskSession::ConstSharedPtr message)
{
  ...
  const bool switching_session =
    message->task_id != active_task_id_ || message->generation != active_generation_;
  if (switching_session && move_ack_tracker_.busy()) {
    // BUG_POINT:STALE_MOVE_ACK_BARRIER - A timed-out placement from an old task can
    // otherwise block every later task because [move,ok] has no session tag.
    RCLCPP_WARN(
      get_logger(), "BUG_POINT:STALE_MOVE_ACK_BARRIER cancelling pending task=%u "
      "generation=%u piece=%u before new task=%u generation=%u",
      move_ack_tracker_.task_id(), move_ack_tracker_.generation(),
      move_ack_tracker_.piece_id(), message->task_id, message->generation);
...
void SerialBridgeNode::on_map_result(const std_msgs::msg::String::ConstSharedPtr message)
{
  if (!serial_enabled_) {
    return;
  }
  if (message->data != "[map,ok]") {
    RCLCPP_WARN(
      get_logger(), "BUG_POINT:MAP_RESULT_FORMAT rejected frame=%s", message->data.c_str());
    return;
  }
  if (move_ack_tracker_.busy()) {
    RCLCPP_WARN(
      get_logger(), "BUG_POINT:MAP_RESULT_ACK_INFLIGHT rejected frame=%s pending_piece=%u",
      message->data.c_str(), move_ack_tracker_.piece_id());
    return;
  }
  enqueue_frame(std::vector<std::uint8_t>(message->data.begin(), message->data.end()), "map_result");
}
  ...
  const vision_interfaces::msg::PuzzlePlacement & message)
{
  ...
  geometry_msgs::msg::Point32 normalized_source;
  geometry_msgs::msg::Point32 normalized_target;
  if (placement_coordinate_frame_ == "camera_center_px") {
  ...
  } else if (placement_coordinate_frame_ == "green_a4_normalized") {
    normalized_source = green_a4_normalized_point(message.source_center_a4_mm);
    normalized_target = green_a4_normalized_point(message.target_center_a4_mm);
...
void SerialBridgeNode::preempt_for_task_command(const int task_number)
{
  const bool cancelled_ack = move_ack_tracker_.busy();
  if (cancelled_ack) {
    move_ack_tracker_.cancel();
  }
  const bool cancelled_deferred = deferred_placement_.has_value();
  deferred_placement_.reset();

  std::size_t discarded_frames = 0U;
  bool partially_written_frame = false;
  for (auto iterator = transmit_queue_.begin(); iterator != transmit_queue_.end(); ) {
    if (iterator->source != "placement") {
      ++iterator;
      continue;
    }
    if (iterator->written_bytes != 0U) {
      partially_written_frame = true;
      ++iterator;
      continue;
    }
    queued_bytes_ -= iterator->bytes.size();
  ...
  // BUG_POINT:TASK_COMMAND_PREEMPT - A valid task frame must cancel the old
  // software session before HMI forwarding or DDS task-session callbacks run.
  RCLCPP_INFO(
    get_logger(),
    "Task command preempted old motion state immediately: new_task=%d ack=%s deferred=%s "
    "discarded_frames=%zu partially_written=%s",
    task_number, cancelled_ack ? "true" : "false",
    cancelled_deferred ? "true" : "false", discarded_frames,
    partially_written_frame ? "true" : "false");
}
...
void SerialBridgeNode::publish_received_frame(const std::string & frame)
{
  ...
  int task_number = 0;
  const bool valid_task_command = serial_bridge_node::parse_task_command(frame, task_number);
  ...
  if (valid_task_command) {
    preempt_for_task_command(task_number);
