...
[task,3]
  ...
  -> 清晰白边优先作为目标外框候选，不作为内部拼接边
  -> 低白度或整边贴黑视为弱白边证据，不单独排除外框候选
...
每块碎片的可信白边都作为外框候选证据，清晰白边不进入内部拼接；但整条边贴黑可能
来自分割或采样误差，只降低白边证据分，不单独否决外框几何候选。多解问题发生在
外框几何候选成立之后。
...
- `challenge_two_white_border_fast_solver_enabled=true` 时，清晰白边作为任务三外框优先
  候选证据，不进入内部拼接；外露边必须覆盖完整矩形四边，弱白边证据只参与排序。
