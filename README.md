...
固定顶部相机
  ...
  -> 完整位于下半区的目标位姿
  -> 控制侧执行
  -> 三题统一闭环复核；发挥题二先小矩形、再同形交换、最后几何回退
```
...
- `puzzle_solver_node`
  ...
  - 发挥题一使用 90--120 mm × 50--90 mm 尺寸范围，目标方向和下半区内中心位置自由；
    ...
    选择，不沿用基础题的严格分差拒绝。
  - 发挥题二先在几何候选中寻找 1--2 块小矩形组合；命中时要求小矩形接缝和最终
    大矩形全部内部接缝的梯度/纹理连续性通过。
  - 若没有小矩形组合，则检测轮廓大小和形状一致的可交换碎片；命中时按发挥题一
    几何逻辑枚举等价解，并选择连接处连续性综合最高的一种。
  - 若小矩形和可交换块均不命中，发挥题二直接退回发挥题一几何求解。
- `puzzle_coordinator_node`
  - 管理启动、扫描、首次求解、缓存整套放置队列、逐片下发和执行完成。
  - 龙门架或电磁铁未离开关键区域时不允许扫描。
  - 首次有效计划中的未放置碎片会缓存为队列；每次控制侧返回 `[move,ok]` 后
    直接下发下一条，不重新拍照，避免机械结构仍在 A4 范围内污染识别。
  - 发挥题二初始求解仍按小矩形优先、同形交换消歧、发挥题一几何回退的顺序选候选。
- `puzzle_geometry`
  - 提供与任务编号无关的单应性、相机中心像素、角度归一化和旋转差计算；控制旋转
    统一为顶视图逆时针正、顺时针负。
- `serial_bridge_node`
  - 作为Jetson侧 UART 唯一拥有者。当前硬件 UART1(`/dev/ttyTHS1`) 回环失败，
    默认 MCU 使用 UART0(`/dev/ttyTHS3`)；串口屏直连角色保留在 UART1。
    MCU口为115200bps 8N1，串口屏口为9600bps 8N1；UART0/UART1可按接线互换，
    但波特率按角色配置。
  - 接收任务命令并把每个 `PuzzlePlacement` 格式化为
    `[x0,y0,x1,y1,angle]`；串口屏可直连Jetson，也可由单片机转发。
  - 把单一待执行请求对应的 `[move,ok]` 映射为带 `piece_id` 的
    `puzzle/placement_done`；协调器收到确认后发送缓存队列中的下一条，
    重复或无请求确认不会推进任务。
- `hmi_control_node`
...
协调节点随后从首次求解缓存的队列中下发下一条，不在每次 `[move,ok]` 后重新拍照。
