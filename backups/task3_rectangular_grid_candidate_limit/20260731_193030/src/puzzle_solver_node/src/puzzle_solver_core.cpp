  ...
  const cv::Point2d & rectified_origin_a4_mm) const
{
    ...
    do {
      ...
      for (int orientation_mask = 0; orientation_mask < orientation_count; ++orientation_mask) {
        ...
        const auto seams = evaluate_internal_seam_continuity(
          poses, pieces, rectified_bgr, pixels_per_mm, rectified_origin_a4_mm);
        if (!seams.pass) {
        ...
        const double score = seams.score + 0.001 * motion_cost;
        if (score < best.score - 1e-9) {
          second_score = best.score;
          best = GridCandidate{score, long_side, short_side, transforms};
        } else if (score < second_score) {
          second_score = score;
        }
      }
  ...
  // rectangular pieces, enumerate grid layouts directly and rank them by all
  // internal seam texture continuity.
  return result;
}
