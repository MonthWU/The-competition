  ...
  const vision_interfaces::msg::PuzzlePlan::ConstSharedPtr message)
{
  ...
  const auto next = std::find_if(
    message->placements.begin(), message->placements.end(),
    [](const auto & placement) {return !placement.already_placed;});
  if (next == message->placements.end()) {
    active_piece_id_ = 0;
    set_state(State::COMPLETE, "ALL_PIECES_VERIFIED");
    return;
  }

  auto session_placement = *next;
  session_placement.task_id = static_cast<std::uint8_t>(active_task_);
  session_placement.generation = task_generation_;
  active_piece_id_ = session_placement.piece_id;
  placement_publisher_->publish(session_placement);
  set_state(State::WAIT_PLACEMENT_DONE, "PLACEMENT_REQUESTED");
}
