  ...
  const cv::Point2d & rectified_origin_a4_mm) const
{
  ...
  bool white_border_constraints_active = false;
  if (config_.white_border_fast_solver_enabled) {
    int white_edge_count = 0;
    int pieces_with_white_edge = 0;
    for (const auto & piece : prepared_pieces) {
      bool piece_has_white_edge = false;
      for (std::size_t edge_index = 0; edge_index < piece.polygon_local_mm.size(); ++edge_index) {
        if (is_white_border_edge(piece, static_cast<int>(edge_index))) {
          ++white_edge_count;
          piece_has_white_edge = true;
        }
      }
      if (piece_has_white_edge) {
        ++pieces_with_white_edge;
      }
    }
    white_border_constraints_active =
      white_edge_count >= 4 &&
      pieces_with_white_edge == static_cast<int>(prepared_pieces.size());
  }
  ...
  const bool white_border_constraints_active) const
{
  ...
  if (white_border_constraints_active) {
    ...
    if (!white_border.valid) {
      // BUG_POINT:CARD_WHITE_BORDER_FRAME - Task 3 must use the detected card
      // white border as the unique outer-frame constraint after contour area
      // has fixed the target rectangle size. Do not fall back to generic
      // geometry if the exposed edges are not the white frame.
      return evaluation;
    }
    ...
    evaluation.direct_accept = white_border.direct_accept;
  } else {
  ...
  const cv::Point2d & rectified_origin_a4_mm) const
{
  ...
  for (const auto & edge : assembly.exposed_edges) {
    ...
    if (!is_white_border_edge(piece, edge.edge_index)) {
      return evaluation;
    }
  ...
  evaluation.score =
    config_.white_border_layout_weight * coverage_error / 100.0 +
