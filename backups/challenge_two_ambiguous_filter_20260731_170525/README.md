...
- `puzzle_solver_node`
  ...
  - 发挥题一使用 90--120 mm × 50--90 mm 尺寸范围，目标方向和下半区内中心位置自由；
    ...
    选择，不沿用基础题的严格分差拒绝。
  - 发挥题二先用扑克牌白边锁定完成矩形外框，白边不得作为内部拼接边；内部搜索
    只处理非白切割边，因此不再把发挥题一的完整未知边枚举作为牌面主路径。
  - 发挥题二只有在白边外框成立后仍存在矩形旋转、全等片互换等多解时，才用连接边
    梯度连续性、中心对称性、对角特殊性和非字符角纯白色度消歧。
