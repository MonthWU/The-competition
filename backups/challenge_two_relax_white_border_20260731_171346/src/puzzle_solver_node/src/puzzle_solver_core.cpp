...
bool PuzzleSolverCore::is_white_border_edge(
  const PieceModel & piece, const int edge_index) const
{
  if (piece.white_border_edge_confidence.empty() || edge_index < 0) {
    return false;
  }
  const auto index = static_cast<std::size_t>(edge_index) %
    piece.white_border_edge_confidence.size();
  return piece.white_border_edge_confidence[index] >= config_.white_border_min_confidence;
}
  ...
  const cv::Point2d & rectified_origin_a4_mm) const
{
  ...
  const bool white_border_constraints_active = config_.white_border_fast_solver_enabled;
            ...
            for (const auto & job : edge_jobs) {
              ...
              for (int variant = 0; variant < variants; ++variant) {
                ...
                if (merge(
                    ...
                    white_border_constraints_active, merged))
                {
  ...
  for (const auto & assembly : states.back()) {
    const RectangleEvaluation rectangle = evaluate_rectangle(
      ...
      white_border_constraints_active);
    if (rectangle.valid) {
  ...
  if (white_border_constraints_active && unique_finals.size() > 1U) {
...
bool PuzzleSolverCore::merge(
  ...
  const bool white_border_constraints_active,
  Assembly & output) const
{
  ...
  if (config_.pattern_enabled && full_edge_match && !rectified_bgr.empty() && pixels_per_mm > 1e-6) {
    const double seam_cost = texture_cost(
      ...
      rectified_bgr, pixels_per_mm, rectified_origin_a4_mm);
    if (white_border_constraints_active &&
      1.0 - seam_cost < config_.seam_gradient_min_continuity)
    {
      return false;
    }
...
PuzzleSolverCore::RectangleEvaluation PuzzleSolverCore::evaluate_rectangle(
  ...
  const cv::Mat & rectified_bgr, const double pixels_per_mm,
  const cv::Point2d & rectified_origin_a4_mm,
  const bool white_border_constraints_active) const
{
  ...
  if (white_border_constraints_active) {
    ...
    if (!white_border.valid) {
      // BUG_POINT:CARD_WHITE_BORDER_FRAME - Task 3 treats clear card white
      // borders as outer-frame evidence after contour area fixes the rectangle.
  ...
  const cv::Point2d & rectified_origin_a4_mm) const
{
  ...
  evaluation.score =
    ...
    (coverage_error + (1.0 - average_white_confidence)) / 100.0;
  return evaluation;
}
