...
bool solver_trace_enabled()
{
  const char * value = std::getenv("PUZZLE_SOLVER_TRACE");
  return value != nullptr && std::string(value) != "0";
}

void trace_rectangular_grid(const std::string & message)
{
  if (solver_trace_enabled()) {
    std::cerr << "BUG_POINT:CHALLENGE_TWO_RECT_GRID_FAST_PATH " << message << '\n';
  }
}
  ...
  const SolverConfig & config)
{
  ...
  constexpr std::size_t kMaxSubsetsPerSide = 512U;
  constexpr std::size_t kMaxPairsPerSide = 2048U;
  auto make_side_pairs = [&](
    const std::vector<SideSubset> & subsets, const double expected) {
      std::vector<SidePair> pairs;
      const std::size_t subset_limit = std::min(subsets.size(), kMaxSubsetsPerSide);
      for (std::size_t first_index = 0; first_index < subset_limit; ++first_index) {
        const auto & first = subsets[first_index];
        for (std::size_t second_index = first_index + 1U;
          second_index < subset_limit; ++second_index)
        {
          const auto & second = subsets[second_index];
          if ((first.mask & second.mask) != 0U ||
            !close_length(first.length_mm, second.length_mm))
          {
            continue;
          }
          const double candidate = 0.5 * (first.length_mm + second.length_mm);
          const double score =
            std::abs(first.length_mm - second.length_mm) / std::max(1e-6, candidate) +
            std::abs(candidate - expected) / std::max(1e-6, expected);
          pairs.push_back(SidePair{first.mask | second.mask, candidate, score});
        }
      }
      std::sort(pairs.begin(), pairs.end(), [](const SidePair & first, const SidePair & second) {
          if (std::abs(first.score - second.score) > 1e-12) {
            return first.score < second.score;
          }
          if (first.mask != second.mask) {
            return first.mask < second.mask;
          }
          return first.length_mm < second.length_mm;
      });
      std::vector<SidePair> unique_pairs;
      unique_pairs.reserve(std::min(pairs.size(), kMaxPairsPerSide));
      std::unordered_set<std::uint64_t> pair_masks;
      for (const auto & pair : pairs) {
        if (!pair_masks.insert(pair.mask).second) {
          continue;
        }
        unique_pairs.push_back(pair);
        if (unique_pairs.size() >= kMaxPairsPerSide) {
          break;
        }
      }
      return unique_pairs;
  ...
  const auto long_pairs = make_side_pairs(long_subsets, expected_long_mm);
  const auto short_pairs = make_side_pairs(short_subsets, expected_short_mm);
  for (const auto & long_pair : long_pairs) {
    if (best.valid && !short_pairs.empty() &&
      long_pair.score + short_pairs.front().score >= best.score)
    {
      break;
    }
    for (const auto & short_pair : short_pairs) {
      const double score_lower_bound = long_pair.score + short_pair.score;
      if (best.valid && score_lower_bound >= best.score) {
        break;
      }
      if ((long_pair.mask & short_pair.mask) != 0U) {
        continue;
      }
      const double candidate_area = std::max(
        1e-6, long_pair.length_mm * short_pair.length_mm);
      const double area_relative_error = std::abs(candidate_area - total_area_mm2) /
        candidate_area;
      if (area_relative_error > area_relative_tolerance) {
        continue;
      }
      const std::uint64_t boundary_mask = long_pair.mask | short_pair.mask;
      ...
      const double balance_tolerance = std::max(
        boundary_tolerance,
        relative_length_tolerance * std::max(long_pair.length_mm, short_pair.length_mm));
      if (!signed_length_balance_possible(unused_edge_lengths, balance_tolerance)) {
        continue;
      }
      const double score = score_lower_bound + area_relative_error;
      if (!best.valid || score < best.score) {
        best.valid = true;
        best.long_mm = long_pair.length_mm;
        best.short_mm = short_pair.length_mm;
        best.score = score;
      }
    }
  }
...
std::vector<PieceModel> PuzzleSolverCore::prepare_pieces(
  const std::vector<PieceModel> & pieces,
  ...
  const cv::Point2d & rectified_origin_a4_mm) const
{
  ...
  for (auto & piece : prepared_pieces) {
    ...
    if (!config_.white_border_fast_solver_enabled) {
      continue;
    }
    if (piece.white_border_edge_confidence.size() != piece.polygon_local_mm.size()) {
      piece.white_border_edge_confidence.assign(piece.polygon_local_mm.size(), -1.0);
    }
    for (std::size_t edge_index = 0; edge_index < piece.polygon_local_mm.size(); ++edge_index) {
      if (piece.white_border_edge_confidence[edge_index] >= 0.0) {
        piece.white_border_edge_confidence[edge_index] =
          std::clamp(piece.white_border_edge_confidence[edge_index], 0.0, 1.0);
        continue;
      }
      piece.white_border_edge_confidence[edge_index] = white_border_edge_score(
        piece, static_cast<int>(edge_index), rectified_bgr, pixels_per_mm,
        rectified_origin_a4_mm);
    }
  }
  ...
  const cv::Point2d & rectified_origin_a4_mm) const
{
  ...
  if (config_.challenge_one_legacy_algorithm_enabled) {
    return solve_challenge_one_edge_enumeration(
      pieces, rectified_bgr, pixels_per_mm, rectified_origin_a4_mm);
  }

  std::vector<PieceModel> prepared_pieces = prepare_pieces(
    pieces, rectified_bgr, pixels_per_mm, rectified_origin_a4_mm);
  const bool white_border_evidence_active =
    config_.white_border_fast_solver_enabled &&
    !config_.challenge_two_specialized_logic_enabled;
  if (config_.challenge_two_specialized_logic_enabled &&
    (prepared_pieces.size() == 2U || prepared_pieces.size() == 4U))
  {
    const auto fast_result = solve_challenge_two_rectangular_grid(
      prepared_pieces, rectified_bgr, pixels_per_mm, rectified_origin_a4_mm);
    if (fast_result.status != "RECTANGULAR_GRID_NOT_APPLICABLE") {
      return fast_result;
    }
  }
        ...
        left_mask = (left_mask - 1) & mask)
      {
        ...
        for (const auto & left : states[static_cast<std::size_t>(left_mask)]) {
          for (const auto & right : states[static_cast<std::size_t>(right_mask)]) {
            ...
            const std::size_t full_edge_jobs = static_cast<std::size_t>(std::count_if(
                edge_jobs.begin(), edge_jobs.end(), [&](const EdgePairJob & job) {
                  return job.relative_error <= config_.edge_length_relative_tolerance;
                }));
            const std::size_t partial_edge_budget = full_edge_jobs > 0U ? 4U : 12U;
            std::size_t partial_edge_jobs = 0U;
            for (const auto & job : edge_jobs) {
              ...
              const bool full_edge_match =
                job.relative_error <= config_.edge_length_relative_tolerance;
              if (!full_edge_match && partial_edge_jobs++ >= partial_edge_budget) {
                // BUG_POINT:EDGE_PAIR_BUDGET - Dense unequal-edge combinations
                // can dominate task-2 runtime. Keep every full-edge candidate,
                // plus the best bounded partial candidates for T-junction fallback.
                continue;
              }
              const int variants = full_edge_match ? 1 : 2;
              for (int variant = 0; variant < variants; ++variant) {
                ...
                if (merge(
                    ...
                    white_border_evidence_active, merged))
                {
  ...
  for (const auto & assembly : states.back()) {
    const RectangleEvaluation rectangle = evaluate_rectangle(
      assembly, prepared_pieces, rectified_bgr, pixels_per_mm, rectified_origin_a4_mm,
      white_border_evidence_active);
    if (rectangle.valid) {
  ...
  if (config_.challenge_two_specialized_logic_enabled) {
    std::vector<FinalCandidate> small_rectangle_finals;
    bool small_rectangle_found = false;
    for (auto & candidate : unique_finals) {
      const auto small_rectangle = evaluate_small_rectangle_priority(
        candidate.assembly, prepared_pieces, rectified_bgr, pixels_per_mm,
        rectified_origin_a4_mm);
      if (!small_rectangle.found) {
        continue;
      }
      small_rectangle_found = true;
      const auto full_seams = evaluate_internal_seam_continuity(
        candidate.assembly.poses, prepared_pieces, rectified_bgr, pixels_per_mm,
        rectified_origin_a4_mm);
      if (!small_rectangle.pass || !full_seams.pass) {
        continue;
      }
      candidate.score =
        small_rectangle.score + full_seams.score + 0.001 * candidate.score;
      small_rectangle_finals.push_back(std::move(candidate));
    }
    if (small_rectangle_found) {
      if (small_rectangle_finals.empty()) {
        // BUG_POINT:CHALLENGE_TWO_SPECIALIZED_LOGIC - Task 3 first locks a
        // 1-2 piece rectangular sub-assembly. If such candidates exist but
        // pattern evidence rejects their seams, do not fall through to a
        // lower-confidence geometry-only layout.
        result.status = "PATTERN_CONTINUITY_REJECTED";
        return result;
      }
      std::sort(small_rectangle_finals.begin(), small_rectangle_finals.end(),
        [](const FinalCandidate & first, const FinalCandidate & second) {
          return first.score < second.score;
        });
      unique_finals = std::move(small_rectangle_finals);
    } else if (has_exchangeable_piece_pair(prepared_pieces)) {
      std::vector<FinalCandidate> exchange_finals;
      for (auto & candidate : unique_finals) {
        const auto full_seams = evaluate_internal_seam_continuity(
          candidate.assembly.poses, prepared_pieces, rectified_bgr, pixels_per_mm,
          rectified_origin_a4_mm);
        ...
        candidate.score = full_seams.score + 0.001 * candidate.score;
        exchange_finals.push_back(std::move(candidate));
      }
      if (exchange_finals.empty()) {
        result.status = "PATTERN_CONTINUITY_REJECTED";
        return result;
      }
      std::sort(exchange_finals.begin(), exchange_finals.end(),
        [](const FinalCandidate & first, const FinalCandidate & second) {
          return first.score < second.score;
        });
      unique_finals = std::move(exchange_finals);
    }
  } else if (white_border_evidence_active && unique_finals.size() > 1U) {
    std::vector<FinalCandidate> filtered_finals;
    for (auto & candidate : unique_finals) {
      const auto ambiguity = evaluate_card_ambiguity(
        candidate.assembly, prepared_pieces, candidate.rectangle.center,
        candidate.rectangle.long_axis, candidate.rectangle.short_axis,
        candidate.rectangle.long_half, candidate.rectangle.short_half,
        rectified_bgr, pixels_per_mm, rectified_origin_a4_mm);
      if (!ambiguity.valid) {
        continue;
      }
      candidate.score += ambiguity.score;
      candidate.rectangle.direct_accept = ambiguity.direct_accept;
      filtered_finals.push_back(std::move(candidate));
    }
    if (filtered_finals.empty()) {
      result.status = "NO_FEASIBLE_PLACEMENT";
      return result;
    }
    std::sort(filtered_finals.begin(), filtered_finals.end(),
      [](const FinalCandidate & first, const FinalCandidate & second) {
        if (first.rectangle.direct_accept != second.rectangle.direct_accept) {
          return first.rectangle.direct_accept;
        }
        return first.score < second.score;
      });
    unique_finals = std::move(filtered_finals);
  }
  ...
  result.score = best.score;
  result.second_score = best.rectangle.direct_accept ? best.score + 1000.0 :
    (unique_finals.size() > 1 ? unique_finals[1].score : best.score + 1000.0);
...
SolveResult PuzzleSolverCore::solve_challenge_one_edge_enumeration(
  const std::vector<PieceModel> & pieces,
  const cv::Mat & rectified_bgr,
  const double pixels_per_mm,
  const cv::Point2d & rectified_origin_a4_mm) const
{
  ...
  std::vector<FinalCandidate> finals;
  std::size_t total_edge_count = 0U;
  for (const auto & piece : prepared_pieces) {
    if (piece.polygon_local_mm.size() < 3U) {
      result.status = "PIECE_GEOMETRY_INVALID";
      return result;
    }
    total_edge_count += piece.polygon_local_mm.size();
  }
  const std::size_t maximum_edges = static_cast<std::size_t>(
    std::max(4, config_.maximum_enumerated_edges));
  if (total_edge_count > maximum_edges) {
    result.status = "EDGE_ENUMERATION_LIMIT_EXCEEDED";
    return result;
  }

  const int full_mask = (1 << static_cast<int>(prepared_pieces.size())) - 1;
  Assembly root = make_leaf(prepared_pieces.front(), 0);
  auto build_children = [&](const Assembly & current, const bool allow_partial_edges) {
      std::vector<Assembly> children;
      std::vector<std::unordered_set<std::string>> child_signatures(
        static_cast<std::size_t>(full_mask + 1));
      for (std::size_t piece_index = 0; piece_index < prepared_pieces.size(); ++piece_index) {
        const int piece_bit = 1 << static_cast<int>(piece_index);
        if ((current.mask & piece_bit) != 0) {
          continue;
        }
        const Assembly leaf = make_leaf(
          prepared_pieces[piece_index], static_cast<int>(piece_index));
        struct EdgePairJob
        {
          std::size_t exposed_edge_index{0U};
          std::size_t piece_edge_index{0U};
          double relative_error{0.0};
          double partial_ratio{0.0};
        };
        std::vector<EdgePairJob> jobs;
        for (std::size_t exposed_index = 0;
          exposed_index < current.exposed_edges.size(); ++exposed_index)
        {
          const double exposed_length = edge_length(current.exposed_edges[exposed_index]);
          for (std::size_t piece_edge_index = 0;
            piece_edge_index < leaf.exposed_edges.size(); ++piece_edge_index)
          {
            const double piece_edge_length = edge_length(leaf.exposed_edges[piece_edge_index]);
            const double maximum_length = std::max(exposed_length, piece_edge_length);
            const double relative_error = std::abs(exposed_length - piece_edge_length) /
              std::max(1e-6, maximum_length);
            const double partial_ratio = std::min(exposed_length, piece_edge_length) /
              std::max(1e-6, maximum_length);
            if (relative_error > config_.edge_length_relative_tolerance) {
            ...
            jobs.push_back(EdgePairJob{
                exposed_index, piece_edge_index, relative_error, partial_ratio});
          }
        }
        std::stable_sort(jobs.begin(), jobs.end(), [&](const auto & first, const auto & second) {
            const bool first_full =
              first.relative_error <= config_.edge_length_relative_tolerance;
            const bool second_full =
              second.relative_error <= config_.edge_length_relative_tolerance;
            if (first_full != second_full) {
              return first_full;
            }
            if (std::abs(first.relative_error - second.relative_error) > 1e-12) {
              return first.relative_error < second.relative_error;
            }
            return first.partial_ratio > second.partial_ratio;
          });
        for (const auto & job : jobs) {
          ...
          for (int variant = 0; variant < variants; ++variant) {
            ...
            if (!merge(
                current, leaf,
                current.exposed_edges[job.exposed_edge_index],
                leaf.exposed_edges[job.piece_edge_index], variant,
                prepared_pieces, rectified_bgr, pixels_per_mm,
                rectified_origin_a4_mm, false, merged))
            {
              continue;
            }
            auto & signatures = child_signatures[static_cast<std::size_t>(merged.mask)];
            if (signatures.insert(assembly_signature(merged)).second) {
              children.push_back(std::move(merged));
            }
          }
        }
      }
  ...
  auto run_enumeration_pass = [&](const bool allow_partial_edges) {
      if (root.mask == full_mask) {
        const RectangleEvaluation rectangle = evaluate_rectangle(
          root, prepared_pieces, rectified_bgr, pixels_per_mm,
          rectified_origin_a4_mm, false);
        if (rectangle.valid) {
          finals.push_back(FinalCandidate{
              root.score + rectangle.score + rectangle.placement_cost,
              rectangle, root});
        }
        return;
      }

      std::vector<Assembly> root_children = build_children(root, allow_partial_edges);
      if (root_children.empty()) {
        return;
      }

      // Each root branch owns its DFS state and output slot. Workers may finish
      // in any order, but results are merged in root-branch order afterwards.
      std::vector<std::vector<FinalCandidate>> finals_by_root(root_children.size());
      std::atomic<std::size_t> next_root_index{0U};
      const int worker_count = std::clamp(config_.enumeration_worker_threads, 1, 5);
      std::vector<std::thread> workers;
      workers.reserve(static_cast<std::size_t>(worker_count));
      for (int worker_index = 0; worker_index < worker_count; ++worker_index) {
        workers.emplace_back([&]() {
            while (true) {
              const std::size_t root_index = next_root_index.fetch_add(
                1U, std::memory_order_relaxed);
              if (root_index >= root_children.size()) {
                return;
              }

              auto & branch_finals = finals_by_root[root_index];
              std::vector<std::unordered_set<std::string>> seen_by_mask(
                static_cast<std::size_t>(full_mask + 1));
              seen_by_mask[static_cast<std::size_t>(root.mask)].insert(
                assembly_signature(root));
              seen_by_mask[static_cast<std::size_t>(root_children[root_index].mask)].insert(
                assembly_signature(root_children[root_index]));

              std::function<void(const Assembly &)> enumerate_branch;
              enumerate_branch = [&](const Assembly & current) {
                  if (current.mask == full_mask) {
                    const RectangleEvaluation rectangle = evaluate_rectangle(
                      current, prepared_pieces, rectified_bgr, pixels_per_mm,
                      rectified_origin_a4_mm, false);
                    if (rectangle.valid) {
                      branch_finals.push_back(FinalCandidate{
                          current.score + rectangle.score + rectangle.placement_cost,
                          rectangle, current});
                    }
                    return;
                  }

                  std::vector<Assembly> children = build_children(
                    current, allow_partial_edges);
                  for (auto & child : children) {
                    auto & seen = seen_by_mask[static_cast<std::size_t>(child.mask)];
                    if (seen.insert(assembly_signature(child)).second) {
                      enumerate_branch(child);
                    }
                  }
                };
              enumerate_branch(root_children[root_index]);
            }
          });
      }
      ...
      for (auto & branch_finals : finals_by_root) {
        finals.insert(
          finals.end(),
          std::make_move_iterator(branch_finals.begin()),
          std::make_move_iterator(branch_finals.end()));
      }
    };
  run_enumeration_pass(false);
  if (finals.empty()) {
    run_enumeration_pass(true);
  }

  // BUG_POINT:CHALLENGE_ONE_EDGE_ENUMERATION - Challenge one anchors piece 0
  // and exhaustively attaches each remaining single piece to every compatible
  // exposed edge. Full-edge solutions are exhausted first; partial/T-junction
  // edges are exhausted only when that pass finds no rectangle. Five fixed
  // C++ workers consume deterministic root branches; recursive levels do not
  // create threads. The edge limit keeps both passes bounded; do not
  // reintroduce score truncation.
  if (finals.empty()) {
    result.status = "NO_FEASIBLE_PLACEMENT";
    return result;
  }
...
SolveResult PuzzleSolverCore::solve_challenge_two_rectangular_grid(
  const std::vector<PieceModel> & pieces,
  const cv::Mat & rectified_bgr, const double pixels_per_mm,
  const cv::Point2d & rectified_origin_a4_mm) const
{
  SolveResult result;
  result.status = "RECTANGULAR_GRID_NOT_APPLICABLE";
  trace_rectangular_grid("enter pieces=" + std::to_string(pieces.size()));
  if (!(pieces.size() == 2U || pieces.size() == 4U)) {
    trace_rectangular_grid("skip piece_count=" + std::to_string(pieces.size()));
    return result;
  }
  ...
  struct RectPiece
  {
    double long_mm{0.0};
    double short_mm{0.0};
    double long_axis_angle_rad{0.0};
  };
  std::vector<RectPiece> rects;
  rects.reserve(pieces.size());
  double average_long = 0.0;
  double average_short = 0.0;
  for (std::size_t piece_index = 0; piece_index < pieces.size(); ++piece_index) {
    const auto & piece = pieces[piece_index];
    if (piece.polygon_local_mm.size() != 4U) {
      trace_rectangular_grid(
        "skip vertices piece=" + std::to_string(piece.id) +
        " count=" + std::to_string(piece.polygon_local_mm.size()));
      return result;
    }
    std::vector<cv::Point2f> points;
    points.reserve(piece.polygon_local_mm.size());
    for (const auto & point : piece.polygon_local_mm) {
      points.emplace_back(static_cast<float>(point.x), static_cast<float>(point.y));
    }
    ...
    if (short_side < 1e-6) {
      trace_rectangular_grid("skip degenerate piece=" + std::to_string(piece.id));
      return result;
    }
    const double area = polygon_area(piece.polygon_local_mm);
    const double rectangularity = area / std::max(1e-6, long_side * short_side);
    if (solver_trace_enabled()) {
      std::ostringstream stream;
      stream << "piece index=" << piece_index << " id=" << piece.id <<
        " long=" << long_side << " short=" << short_side <<
        " polygon_area=" << area << " msg_area=" << piece.area_mm2 <<
        " rectangularity=" << rectangularity;
      trace_rectangular_grid(stream.str());
    }
    if (rectangularity + 1e-9 < config_.small_rectangle_min_rectangularity) {
      trace_rectangular_grid(
        "skip rectangularity piece=" + std::to_string(piece.id) +
        " threshold=" + std::to_string(config_.small_rectangle_min_rectangularity));
      return result;
    }
    ...
    rects.push_back(RectPiece{long_side, short_side, long_angle});
    average_long += long_side;
    average_short += short_side;
  }
  average_long /= static_cast<double>(rects.size());
  average_short /= static_cast<double>(rects.size());
  const double relative_tolerance =
    std::max(config_.same_shape_relative_tolerance, config_.edge_length_relative_tolerance);
  for (const auto & rect : rects) {
    if (std::abs(rect.long_mm - average_long) >
      std::max(1.0, relative_tolerance * std::max(rect.long_mm, average_long)) ||
      std::abs(rect.short_mm - average_short) >
      std::max(1.0, relative_tolerance * std::max(rect.short_mm, average_short)))
    {
      std::ostringstream stream;
      stream << "skip same_shape average_long=" << average_long <<
        " average_short=" << average_short << " rect_long=" << rect.long_mm <<
        " rect_short=" << rect.short_mm << " tolerance=" << relative_tolerance;
      trace_rectangular_grid(stream.str());
      return result;
    }
  }
  {
    std::ostringstream stream;
    stream << "same_shape pass average_long=" << average_long <<
      " average_short=" << average_short << " tolerance=" << relative_tolerance;
    trace_rectangular_grid(stream.str());
  }
  ...
  struct GridShape
  {
    int columns{1};
    int rows{1};
    bool long_axis_vertical{true};
  };
  std::vector<GridShape> grids;
  if (pieces.size() == 2U) {
    grids.push_back(GridShape{2, 1, true});
    grids.push_back(GridShape{1, 2, true});
    grids.push_back(GridShape{2, 1, false});
    grids.push_back(GridShape{1, 2, false});
  } else {
    grids.push_back(GridShape{2, 2, true});
    grids.push_back(GridShape{2, 2, false});
  }
  ...
  std::vector<PendingGridCandidate> pending_candidates;
  GridCandidate best;
  double second_score = std::numeric_limits<double>::infinity();
  bool saw_grid = false;
  bool saw_continuity_rejection = false;

  std::vector<int> order(pieces.size());
  std::iota(order.begin(), order.end(), 0);
  for (const auto & grid : grids) {
    const double cell_width = grid.long_axis_vertical ? average_short : average_long;
    const double cell_height = grid.long_axis_vertical ? average_long : average_short;
    const double target_width = cell_width * grid.columns;
    const double target_height = cell_height * grid.rows;
    const double long_side = std::max(target_width, target_height);
    const double short_side = std::min(target_width, target_height);
    ...
    {
      std::ostringstream stream;
      stream << "skip grid target columns=" << grid.columns << " rows=" << grid.rows <<
        " vertical=" << grid.long_axis_vertical << " long=" << long_side <<
        " short=" << short_side;
      trace_rectangular_grid(stream.str());
      continue;
    }
    saw_grid = true;
    cv::Point2d target_center(config_.target_center_x_a4_mm, config_.target_center_y_a4_mm);
    if (config_.free_target_pose_enabled) {
      target_center = cv::Point2d(
        config_.placement_frame_origin_x_mm + 0.5 * config_.placement_frame_width_mm,
        config_.placement_frame_origin_y_mm + 0.5 * config_.placement_frame_height_mm);
    }
    if (config_.free_target_pose_enabled) {
      const double margin = std::max(0.0, config_.placement_frame_margin_mm);
      const double frame_min_x = config_.placement_frame_origin_x_mm + margin;
      const double frame_min_y = config_.placement_frame_origin_y_mm + margin;
      const double frame_max_x =
        config_.placement_frame_origin_x_mm + config_.placement_frame_width_mm - margin;
      const double frame_max_y =
        config_.placement_frame_origin_y_mm + config_.placement_frame_height_mm - margin;
      if (target_width > frame_max_x - frame_min_x + 1e-6 ||
        target_height > frame_max_y - frame_min_y + 1e-6)
      {
        trace_rectangular_grid("skip grid outside placement frame");
        continue;
      }
      target_center.x = 0.5 * (frame_min_x + frame_max_x);
      target_center.y = 0.5 * (frame_min_y + frame_max_y);
    }

    std::sort(order.begin(), order.end());
    do {
      const int orientation_count = 1 << static_cast<int>(pieces.size());
      for (int orientation_mask = 0; orientation_mask < orientation_count; ++orientation_mask) {
        std::vector<PiecePose> poses;
        poses.reserve(pieces.size());
        std::vector<RigidTransform> transforms(pieces.size());
        for (std::size_t slot = 0; slot < order.size(); ++slot) {
          const int piece_index = order[slot];
          const int column = static_cast<int>(slot) % grid.columns;
          const int row = static_cast<int>(slot) / grid.columns;
          const cv::Point2d cell_center(
            target_center.x - 0.5 * target_width + cell_width * (column + 0.5),
            target_center.y - 0.5 * target_height + cell_height * (row + 0.5));
          const double target_long_angle = grid.long_axis_vertical ? kPi / 2.0 : 0.0;
          const double flip = (orientation_mask & (1 << piece_index)) != 0 ? kPi : 0.0;
          RigidTransform transform;
          transform.angle_rad =
            target_long_angle + flip - rects[static_cast<std::size_t>(piece_index)].long_axis_angle_rad;
          transform.translation = cell_center;
          transforms[static_cast<std::size_t>(piece_index)] = transform;
          poses.push_back(PiecePose{piece_index, transform});
        }

        const double motion_cost = std::accumulate(
          poses.begin(), poses.end(), 0.0,
          [&](const double sum, const PiecePose & pose) {
            const auto & piece = pieces[static_cast<std::size_t>(pose.piece_index)];
            return sum +
                   config_.anchor_move_distance_weight *
                   cv::norm(pose.transform.translation - piece.source_center_a4_mm) +
                   config_.anchor_rotation_weight *
                   std::abs(normalized_angle_difference_deg(
                     pose.transform.angle_rad * 180.0 / kPi, 0.0));
          });
        pending_candidates.push_back(PendingGridCandidate{
            motion_cost, long_side, short_side, std::move(poses), std::move(transforms)});
      }
    } while (std::next_permutation(order.begin(), order.end()));
  }

  if (!saw_grid) {
    trace_rectangular_grid("skip no_target_grid");
    return result;
  }
  std::sort(pending_candidates.begin(), pending_candidates.end(),
    [](const PendingGridCandidate & first, const PendingGridCandidate & second) {
      return first.motion_cost < second.motion_cost;
    });
  const std::size_t texture_candidate_limit = config_.pattern_enabled ?
    std::min<std::size_t>(8U, pending_candidates.size()) : pending_candidates.size();
  {
    std::ostringstream stream;
    stream << "pending=" << pending_candidates.size() <<
      " texture_evaluated=" << texture_candidate_limit;
    trace_rectangular_grid(stream.str());
  }
  for (std::size_t index = 0; index < texture_candidate_limit; ++index) {
    const auto & candidate = pending_candidates[index];
    const auto seams = evaluate_internal_seam_continuity(
      candidate.poses, pieces, rectified_bgr, pixels_per_mm, rectified_origin_a4_mm);
    if (!seams.pass) {
      saw_continuity_rejection = true;
      continue;
    }
    const double score = seams.score + 0.001 * candidate.motion_cost;
    if (score < best.score - 1e-9) {
      second_score = best.score;
      best = GridCandidate{
        score, candidate.width_mm, candidate.height_mm, candidate.transforms};
    } else if (score < second_score) {
      second_score = score;
    }
  }
  if (best.transforms.empty()) {
    result.status = saw_continuity_rejection ?
      "PATTERN_CONTINUITY_REJECTED" : "NO_FEASIBLE_PLACEMENT";
    trace_rectangular_grid("reject status=" + result.status);
    return result;
  }
  result.solved = true;
  result.status = "SOLVED";
  result.target_width_mm = best.width_mm;
  result.target_height_mm = best.height_mm;
  result.score = best.score;
  result.second_score = std::isfinite(second_score) ? second_score : best.score + 1000.0;
  result.target_pose_by_piece = std::move(best.transforms);
  trace_rectangular_grid("solved score=" + std::to_string(result.score));
  // BUG_POINT:CHALLENGE_TWO_RECT_GRID_FAST_PATH - Rectangular playing-card
  // pieces can make the generic edge-chain DP explode. For 2/4 same-shaped
  // rectangular pieces, enumerate grid layouts directly, keep the lowest-motion
  // candidates, and rank that small set by internal seam texture continuity.
  return result;
}
...
{
  (void)white_border_evidence_active;
  const double left_length = edge_length(left_edge);
  const double right_length = edge_length(right_edge);
  ...
  output.seam_continuities = left.seam_continuities;
  output.seam_continuities.insert(
    output.seam_continuities.end(), right.seam_continuities.begin(),
    right.seam_continuities.end());
  ...
  if (seam_continuity >= 0.0) {
    output.seam_continuities.push_back(std::clamp(seam_continuity, 0.0, 1.0));
  }
...
PuzzleSolverCore::RectangleEvaluation PuzzleSolverCore::evaluate_rectangle(
  ...
  const cv::Mat & rectified_bgr, const double pixels_per_mm,
  const cv::Point2d & rectified_origin_a4_mm,
  const bool white_border_evidence_active) const
{
  ...
  evaluation.long_axis = long_axis;
  evaluation.short_axis = short_axis;
  evaluation.long_half = long_half;
  evaluation.short_half = short_half;
  const double boundary_tolerance = std::max(
    std::max(0.0, config_.boundary_edge_tolerance_mm), config_.endpoint_tolerance_mm);
  LengthAreaPartitionResult length_area;
  if (white_border_evidence_active) {
    const auto white_border = evaluate_white_border_layout(
      assembly, pieces, center, long_axis, short_axis, long_half, short_half,
      rectified_bgr, pixels_per_mm, rectified_origin_a4_mm);
    if (!white_border.valid) {
      // BUG_POINT:CARD_WHITE_BORDER_FRAME - Task 3 treats card border samples
      // as outer-frame evidence after contour area fixes the rectangle.
      // A weak or black-looking border sample may be recognition error, so it
      // should not by itself reject an otherwise valid geometric candidate.
      return evaluation;
    }
    length_area.valid = true;
    length_area.long_mm = long_side;
    length_area.short_mm = short_side;
    length_area.score = white_border.score;
  } else {
    length_area = evaluate_length_area_partition(
...
double PuzzleSolverCore::white_border_edge_score(
  ...
  const cv::Point2d & rectified_origin_a4_mm) const
{
  ...
  auto pixel_whiteness = [&](const cv::Vec3b & bgr) {
      const double blue = bgr[0];
      const double green = bgr[1];
      const double red = bgr[2];
      const double luma = 0.114 * blue + 0.587 * green + 0.299 * red;
      const double chroma = std::max({blue, green, red}) - std::min({blue, green, red});
      const double luma_score = std::clamp(
        (luma - config_.white_border_min_luma) /
        std::max(1.0, 255.0 - config_.white_border_min_luma), 0.0, 1.0);
      // Locked exposure can render card white near luma 130; keep chroma in the score.
      const double dim_paper_luma_score = std::clamp((luma - 85.0) / 50.0, 0.0, 1.0);
      const double chroma_score = std::clamp(
        1.0 - chroma / std::max(1.0, config_.white_border_max_chroma), 0.0, 1.0);
      const double paper_luma_score = std::max(luma_score, dim_paper_luma_score);
      return 0.65 * paper_luma_score + 0.35 * chroma_score;
    };

  double score_sum = 0.0;
  int sample_count = 0;
  const int along_samples = std::max(6, config_.texture_samples_along_edge);
  const int across_samples = std::max(1, config_.texture_samples_across_edge);
  for (int along_index = 0; along_index < along_samples; ++along_index) {
    const double ratio = (along_index + 0.5) / along_samples;
    const cv::Point2d edge_point = first + ratio * (second - first);
    for (int across_index = 1; across_index <= across_samples; ++across_index) {
      const double distance = std::max(0.5, config_.white_border_band_mm) *
        across_index / across_samples;
      ...
      if (sample_px.x < 0.0F || sample_px.y < 0.0F ||
        sample_px.x >= rectified_bgr.cols || sample_px.y >= rectified_bgr.rows)
      {
        continue;
      }
      cv::Mat sample;
      cv::getRectSubPix(rectified_bgr, cv::Size(1, 1), sample_px, sample);
      score_sum += pixel_whiteness(sample.at<cv::Vec3b>(0, 0));
      ++sample_count;
    }
  }
  if (sample_count < 4) {
  ...
  return std::clamp(score_sum / static_cast<double>(sample_count), 0.0, 1.0);
}

PuzzleSolverCore::WhiteBorderEvaluation PuzzleSolverCore::evaluate_white_border_layout(
  const Assembly & assembly, const std::vector<PieceModel> & pieces,
  const cv::Point2d & center, const cv::Point2d & long_axis,
  const cv::Point2d & short_axis, const double long_half, const double short_half,
  const cv::Mat & rectified_bgr, const double pixels_per_mm,
  const cv::Point2d & rectified_origin_a4_mm) const
{
  (void)rectified_bgr;
  (void)pixels_per_mm;
  (void)rectified_origin_a4_mm;
  WhiteBorderEvaluation evaluation;
  const double long_side = 2.0 * long_half;
  const double short_side = 2.0 * short_half;
  const double boundary_tolerance = std::max(
    std::max(0.0, config_.boundary_edge_tolerance_mm), config_.endpoint_tolerance_mm);
  std::array<double, 4> coverage{0.0, 0.0, 0.0, 0.0};
  double white_confidence_sum = 0.0;
  double white_confidence_length = 0.0;

  for (const auto & edge : assembly.exposed_edges) {
    const auto & piece = pieces[static_cast<std::size_t>(edge.piece_index)];
    const double length = edge_length(edge);
    double confidence = 0.0;
    if (!piece.white_border_edge_confidence.empty()) {
      const auto confidence_index = static_cast<std::size_t>(edge.edge_index) %
        piece.white_border_edge_confidence.size();
      confidence = std::clamp(piece.white_border_edge_confidence[confidence_index], 0.0, 1.0);
    }
    white_confidence_sum += confidence * length;
    white_confidence_length += length;
    const cv::Point2d first = edge.first - center;
    const cv::Point2d second = edge.second - center;
    const double first_long = first.dot(long_axis);
    const double second_long = second.dot(long_axis);
    const double first_short = first.dot(short_axis);
    const double second_short = second.dot(short_axis);
    const double projected_long = std::abs(second_long - first_long);
    const double projected_short = std::abs(second_short - first_short);

    if (std::abs(first_short - short_half) <= boundary_tolerance &&
      std::abs(second_short - short_half) <= boundary_tolerance &&
      std::abs(first_long) <= long_half + boundary_tolerance &&
      std::abs(second_long) <= long_half + boundary_tolerance)
    {
      coverage[0] += projected_long;
    } else if (std::abs(first_short + short_half) <= boundary_tolerance &&
      std::abs(second_short + short_half) <= boundary_tolerance &&
      std::abs(first_long) <= long_half + boundary_tolerance &&
      std::abs(second_long) <= long_half + boundary_tolerance)
    {
      coverage[1] += projected_long;
    } else if (std::abs(first_long - long_half) <= boundary_tolerance &&
      std::abs(second_long - long_half) <= boundary_tolerance &&
      std::abs(first_short) <= short_half + boundary_tolerance &&
      std::abs(second_short) <= short_half + boundary_tolerance)
    {
      coverage[2] += projected_short;
    } else if (std::abs(first_long + long_half) <= boundary_tolerance &&
      std::abs(second_long + long_half) <= boundary_tolerance &&
      std::abs(first_short) <= short_half + boundary_tolerance &&
      std::abs(second_short) <= short_half + boundary_tolerance)
    {
      coverage[3] += projected_short;
    } else {
      return evaluation;
    }
  }
  ...
  const double coverage_error =
    std::abs(coverage[0] - long_side) / std::max(1e-6, long_side) +
    std::abs(coverage[1] - long_side) / std::max(1e-6, long_side) +
    std::abs(coverage[2] - short_side) / std::max(1e-6, short_side) +
    std::abs(coverage[3] - short_side) / std::max(1e-6, short_side);
  const double allowed_coverage_error =
    4.0 * boundary_tolerance / std::max(1e-6, std::min(long_side, short_side));
  if (coverage_error > allowed_coverage_error) {
    return evaluation;
  }

  const double average_white_confidence = white_confidence_length > 1e-6 ?
    white_confidence_sum / white_confidence_length : 0.0;
  const double white_confidence_error =
    1.0 - std::clamp(
    average_white_confidence / std::max(1e-6, config_.white_border_min_confidence), 0.0, 1.0);
  evaluation.valid = true;
  evaluation.score =
    config_.white_border_layout_weight *
    (coverage_error + white_confidence_error) / 100.0;
  return evaluation;
}

PuzzleSolverCore::AmbiguityEvaluation PuzzleSolverCore::evaluate_card_ambiguity(
  const Assembly & assembly, const std::vector<PieceModel> & pieces,
  const cv::Point2d & center, const cv::Point2d & long_axis,
  const cv::Point2d & short_axis, const double long_half, const double short_half,
  const cv::Mat & rectified_bgr, const double pixels_per_mm,
  const cv::Point2d & rectified_origin_a4_mm) const
{
  AmbiguityEvaluation evaluation;
  double seam_score = 0.0;
  if (!assembly.seam_continuities.empty()) {
    double continuity_sum = 0.0;
    for (const double continuity : assembly.seam_continuities) {
      if (continuity + 1e-9 < config_.seam_gradient_min_continuity) {
        return evaluation;
      }
      continuity_sum += continuity;
    }
    const double average_continuity =
      continuity_sum / static_cast<double>(assembly.seam_continuities.size());
    seam_score = config_.texture_weight * (1.0 - average_continuity) / 100.0;
  }
  auto sample_assembly_bgr = [&](const cv::Point2d & point, cv::Vec3d & bgr) {
      if (rectified_bgr.empty() || pixels_per_mm <= 1e-6) {
        return false;
      }
      for (const auto & pose : assembly.poses) {
        const auto & piece = pieces[static_cast<std::size_t>(pose.piece_index)];
        ...
        polygon_f.reserve(polygon.size());
        for (const auto & vertex : polygon) {
          polygon_f.emplace_back(static_cast<float>(vertex.x), static_cast<float>(vertex.y));
        }
        if (cv::pointPolygonTest(
            polygon_f, cv::Point2f(static_cast<float>(point.x), static_cast<float>(point.y)),
            false) < -0.25)
        {
          continue;
        }
        const cv::Point2d local = rotate_point(
          point - pose.transform.translation, -pose.transform.angle_rad);
        const cv::Point2d source_mm = local + piece.source_center_a4_mm;
        const cv::Point2f source_px(
          static_cast<float>((source_mm.x - rectified_origin_a4_mm.x) * pixels_per_mm),
          static_cast<float>((source_mm.y - rectified_origin_a4_mm.y) * pixels_per_mm));
        if (source_px.x < 0.0F || source_px.y < 0.0F ||
          source_px.x >= rectified_bgr.cols || source_px.y >= rectified_bgr.rows)
        {
          return false;
        }
        cv::Mat sample;
        cv::getRectSubPix(rectified_bgr, cv::Size(1, 1), source_px, sample);
        const cv::Vec3b value = sample.at<cv::Vec3b>(0, 0);
        bgr = cv::Vec3d(value[0], value[1], value[2]);
        return true;
      }
      return false;
    };
  auto pixel_whiteness = [&](const cv::Vec3d & bgr) {
      const double luma = 0.114 * bgr[0] + 0.587 * bgr[1] + 0.299 * bgr[2];
      const double chroma = std::max({bgr[0], bgr[1], bgr[2]}) -
        std::min({bgr[0], bgr[1], bgr[2]});
      const double luma_score = std::clamp(
        (luma - config_.white_border_min_luma) /
        std::max(1.0, 255.0 - config_.white_border_min_luma), 0.0, 1.0);
      const double chroma_score = std::clamp(
        1.0 - chroma / std::max(1.0, config_.white_border_max_chroma), 0.0, 1.0);
      return std::min(luma_score, chroma_score);
    };

  double center_symmetry = 0.5;
  if (!rectified_bgr.empty()) {
    double similarity_sum = 0.0;
    int similarity_count = 0;
    const double margin = std::min({5.0, long_half * 0.25, short_half * 0.25});
    for (int y_index = 0; y_index < 8; ++y_index) {
      const double y = -short_half + margin +
        (2.0 * (short_half - margin)) * (y_index + 0.5) / 8.0;
      for (int x_index = 0; x_index < 12; ++x_index) {
        const double x = -long_half + margin +
          (2.0 * (long_half - margin)) * (x_index + 0.5) / 12.0;
        const cv::Point2d first = center + long_axis * x + short_axis * y;
        const cv::Point2d second = center - long_axis * x - short_axis * y;
        cv::Vec3d first_bgr;
        cv::Vec3d second_bgr;
        if (!sample_assembly_bgr(first, first_bgr) ||
          !sample_assembly_bgr(second, second_bgr))
        {
          continue;
        }
        similarity_sum += std::clamp(1.0 - cv::norm(first_bgr - second_bgr) / 441.67295593,
          0.0, 1.0);
        ++similarity_count;
      }
    }
    if (similarity_count >= 12) {
      center_symmetry = similarity_sum / static_cast<double>(similarity_count);
    }
  }
  if (center_symmetry + 1e-9 < config_.center_symmetry_pass_threshold) {
    return evaluation;
  }

  double non_character_white = 1.0;
  double diagonal_specialness = 0.0;
  if (!rectified_bgr.empty()) {
    std::array<double, 4> corner_whiteness{0.0, 0.0, 0.0, 0.0};
    const std::array<cv::Point2d, 4> corner_signs{
      cv::Point2d{-1.0, -1.0}, cv::Point2d{1.0, -1.0},
      cv::Point2d{1.0, 1.0}, cv::Point2d{-1.0, 1.0}};
    const double patch = std::max(2.0, config_.card_corner_sample_mm);
    for (std::size_t corner = 0; corner < corner_signs.size(); ++corner) {
      ...
      for (int y_index = 0; y_index < 5; ++y_index) {
        for (int x_index = 0; x_index < 5; ++x_index) {
          const double x = corner_signs[corner].x *
            (long_half - patch * (x_index + 0.5) / 5.0);
          const double y = corner_signs[corner].y *
            (short_half - patch * (y_index + 0.5) / 5.0);
          cv::Vec3d bgr;
          if (sample_assembly_bgr(center + long_axis * x + short_axis * y, bgr)) {
            white_sum += pixel_whiteness(bgr);
            ++white_count;
          }
        }
      }
      corner_whiteness[corner] = white_count > 0 ?
        white_sum / static_cast<double>(white_count) : 0.0;
    }
    const double first_diagonal = 0.5 * (corner_whiteness[0] + corner_whiteness[2]);
    const double second_diagonal = 0.5 * (corner_whiteness[1] + corner_whiteness[3]);
    non_character_white = std::max(first_diagonal, second_diagonal);
    diagonal_specialness = std::abs(first_diagonal - second_diagonal);
  }
  if (non_character_white + 1e-9 < config_.non_character_corner_white_pass_threshold) {
    return evaluation;
  }

  evaluation.valid = true;
  evaluation.direct_accept =
    center_symmetry >= config_.center_symmetry_direct_threshold;
  evaluation.score =
    seam_score +
    config_.center_symmetry_weight * (1.0 - center_symmetry) / 100.0 +
    config_.non_character_corner_white_weight * (1.0 - non_character_white) / 100.0 -
    config_.diagonal_specialness_weight * diagonal_specialness / 100.0;
  return evaluation;
}

PuzzleSolverCore::RectangleShapeEvaluation PuzzleSolverCore::evaluate_rectangle_shape(
  const std::vector<PiecePose> & poses,
  const std::vector<PieceModel> & pieces) const
{
  RectangleShapeEvaluation evaluation;
  std::vector<cv::Point2f> all_points;
  double total_area = 0.0;
  ...
  if (all_points.size() < 3U || total_area <= 1e-6) {
    return evaluation;
  }
  ...
  const double rectangle_area = std::max(1e-6, long_side * short_side);
  const double rectangularity = std::clamp(total_area / rectangle_area, 0.0, 1.0);
  const double aspect_ratio = long_side / std::max(1e-6, short_side);
  evaluation.rectangularity = rectangularity;
  evaluation.long_side = long_side;
  evaluation.short_side = short_side;
  evaluation.score =
    config_.rectangularity_weight * std::abs(1.0 - rectangularity) / 100.0;
  evaluation.valid =
    rectangularity + 1e-9 >= config_.small_rectangle_min_rectangularity &&
    aspect_ratio + 1e-9 >= config_.small_rectangle_min_aspect_ratio &&
    long_side <= config_.target_long_max_mm + config_.endpoint_tolerance_mm &&
    short_side <= config_.target_short_max_mm + config_.endpoint_tolerance_mm;
  return evaluation;
}

PuzzleSolverCore::SeamContinuityEvaluation PuzzleSolverCore::evaluate_internal_seam_continuity(
  const std::vector<PiecePose> & poses,
  const std::vector<PieceModel> & pieces,
  const cv::Mat & rectified_bgr, const double pixels_per_mm,
  const cv::Point2d & rectified_origin_a4_mm) const
{
  SeamContinuityEvaluation evaluation;
  if (!config_.pattern_enabled || rectified_bgr.empty() || pixels_per_mm <= 1e-6) {
    return evaluation;
  }
  evaluation.evidence_available = true;
  evaluation.minimum = 1.0;
  ...
  struct TransformedEdge
  {
    int piece_index{-1};
    int edge_index{-1};
    cv::Point2d first{0.0, 0.0};
    cv::Point2d second{0.0, 0.0};
    cv::Point2d centroid{0.0, 0.0};
  };

  std::vector<TransformedEdge> edges;
  for (const auto & pose : poses) {
    const auto & piece = pieces[static_cast<std::size_t>(pose.piece_index)];
    const auto polygon = transformed_polygon(piece, pose.transform);
    const cv::Point2d centroid = polygon_centroid(polygon);
    for (std::size_t edge_index = 0; edge_index < polygon.size(); ++edge_index) {
      edges.push_back(TransformedEdge{
          pose.piece_index,
          static_cast<int>(edge_index),
          polygon[edge_index],
          polygon[(edge_index + 1U) % polygon.size()],
          centroid});
    }
  }

  double continuity_sum = 0.0;
  const double line_tolerance = std::max(1.0, config_.endpoint_tolerance_mm);
  for (std::size_t first_index = 0; first_index < edges.size(); ++first_index) {
    const auto & first = edges[first_index];
    const cv::Point2d first_direction = first.second - first.first;
    const double first_length = cv::norm(first_direction);
    if (first_length < 1e-6) {
      continue;
    }
    ...
    const double first_min = std::min(first.first.dot(tangent), first.second.dot(tangent));
    const double first_max = std::max(first.first.dot(tangent), first.second.dot(tangent));
    for (std::size_t second_index = first_index + 1U; second_index < edges.size(); ++second_index) {
      ...
      const cv::Point2d second_direction = second.second - second.first;
      const double second_length = cv::norm(second_direction);
      if (second_length < 1e-6) {
        continue;
      }
      const cv::Point2d second_tangent = second_direction * (1.0 / second_length);
      if (std::abs(cross_2d(tangent, second_tangent)) > 0.05 ||
        std::abs(cross_2d(tangent, second.first - first.first)) > line_tolerance ||
        std::abs(cross_2d(tangent, second.second - first.first)) > line_tolerance)
      {
        continue;
      }
      const double first_side = (first.centroid - first.first).dot(normal);
      const double second_side = (second.centroid - first.first).dot(normal);
      if (first_side * second_side >= 0.0) {
        continue;
      }
      const double second_min = std::min(second.first.dot(tangent), second.second.dot(tangent));
      const double second_max = std::max(second.first.dot(tangent), second.second.dot(tangent));
      const double overlap = std::min(first_max, second_max) - std::max(first_min, second_min);
      if (overlap <= std::max(1.0, config_.endpoint_tolerance_mm)) {
        continue;
      }
      const double first_cost = texture_cost(
        pieces[static_cast<std::size_t>(first.piece_index)], first.edge_index,
        pieces[static_cast<std::size_t>(second.piece_index)], second.edge_index,
        rectified_bgr, pixels_per_mm, rectified_origin_a4_mm);
      const double second_cost = texture_cost(
        pieces[static_cast<std::size_t>(second.piece_index)], second.edge_index,
        pieces[static_cast<std::size_t>(first.piece_index)], first.edge_index,
        rectified_bgr, pixels_per_mm, rectified_origin_a4_mm);
      const double continuity = std::clamp(1.0 - std::min(first_cost, second_cost), 0.0, 1.0);
      evaluation.minimum = std::min(evaluation.minimum, continuity);
      continuity_sum += continuity;
      ++evaluation.seam_count;
      if (continuity + 1e-9 < config_.seam_gradient_min_continuity) {
  ...
  if (evaluation.seam_count > 0) {
    evaluation.average = continuity_sum / static_cast<double>(evaluation.seam_count);
    evaluation.score = config_.texture_weight * (1.0 - evaluation.average) / 100.0;
  }
  return evaluation;
}

PuzzleSolverCore::SmallRectangleEvaluation PuzzleSolverCore::evaluate_small_rectangle_priority(
  const Assembly & assembly, const std::vector<PieceModel> & pieces,
  const cv::Mat & rectified_bgr, const double pixels_per_mm,
  const cv::Point2d & rectified_origin_a4_mm) const
{
  SmallRectangleEvaluation best;
  const int pose_count = static_cast<int>(assembly.poses.size());
  for (int mask = 1; mask < (1 << pose_count); ++mask) {
    const int subset_size = bit_count(mask);
    if (subset_size > 2) {
      continue;
    }
    std::vector<PiecePose> subset;
    subset.reserve(static_cast<std::size_t>(subset_size));
    for (int index = 0; index < pose_count; ++index) {
      if ((mask & (1 << index)) != 0) {
        subset.push_back(assembly.poses[static_cast<std::size_t>(index)]);
      }
    }
    const auto rectangle = evaluate_rectangle_shape(subset, pieces);
    if (!rectangle.valid) {
      continue;
    }
    best.found = true;
    const auto seams = evaluate_internal_seam_continuity(
      subset, pieces, rectified_bgr, pixels_per_mm, rectified_origin_a4_mm);
    if (!seams.pass) {
      continue;
    }
    const double score = rectangle.score + seams.score;
    if (!best.pass || score < best.score) {
      best.pass = true;
      best.score = score;
    }
  }
...
bool PuzzleSolverCore::has_exchangeable_piece_pair(
  const std::vector<PieceModel> & pieces) const
{
  for (std::size_t first = 0; first < pieces.size(); ++first) {
    for (std::size_t second = first + 1U; second < pieces.size(); ++second) {
      if (equivalent_piece_shape(pieces[first], pieces[second])) {
        return true;
      }
    }
  }
...
bool PuzzleSolverCore::equivalent_piece_shape(
  const PieceModel & first, const PieceModel & second) const
{
  const auto & first_polygon = first.polygon_local_mm;
  const auto & second_polygon = second.polygon_local_mm;
  const std::size_t count = first_polygon.size();
  if (count < 3U || count != second_polygon.size()) {
    return false;
  }
  const double first_area = first.area_mm2 > 0.0 ? first.area_mm2 : polygon_area(first_polygon);
  const double second_area = second.area_mm2 > 0.0 ? second.area_mm2 : polygon_area(second_polygon);
  const double relative_area_error = std::abs(first_area - second_area) /
    std::max(1e-6, std::max(first_area, second_area));
  if (relative_area_error > config_.same_shape_relative_tolerance) {
    return false;
  }

  auto features = [](const std::vector<cv::Point2d> & polygon) {
      ...
      output.lengths.reserve(size);
      output.angles_deg.reserve(size);
      for (std::size_t index = 0; index < size; ++index) {
        const cv::Point2d previous = polygon[(index + size - 1U) % size];
        const cv::Point2d current = polygon[index];
        const cv::Point2d next = polygon[(index + 1U) % size];
        const cv::Point2d edge = next - current;
        output.lengths.push_back(cv::norm(edge));
        const cv::Point2d incoming = previous - current;
        const cv::Point2d outgoing = next - current;
        const double denominator = std::max(1e-9, cv::norm(incoming) * cv::norm(outgoing));
        const double cosine = std::clamp(incoming.dot(outgoing) / denominator, -1.0, 1.0);
        output.angles_deg.push_back(std::acos(cosine) * 180.0 / kPi);
      }
      return output;
    };

  const auto first_features = features(first_polygon);
  const auto second_features = features(second_polygon);
  const double relative_tolerance =
    std::max(config_.same_shape_relative_tolerance, config_.edge_length_relative_tolerance);
  const double angle_tolerance = std::max(0.0, config_.same_shape_angle_tolerance_deg);
  auto close_length = [&](const double left, const double right) {
      return std::abs(left - right) <=
             std::max(0.5, relative_tolerance * std::max(left, right));
    };
  auto close_angle = [&](const double left, const double right) {
      return std::abs(left - right) <= angle_tolerance;
    };
  ...
  for (std::size_t shift = 0; shift < count; ++shift) {
    ...
    for (std::size_t index = 0; index < count; ++index) {
      const std::size_t forward = (shift + index) % count;
      const std::size_t reverse_vertex = (shift + count - index) % count;
      const std::size_t reverse_edge = (shift + count - index - 1U) % count;
      forward_match = forward_match &&
        close_length(first_features.lengths[index], second_features.lengths[forward]) &&
        close_angle(first_features.angles_deg[index], second_features.angles_deg[forward]);
      reverse_match = reverse_match &&
        close_length(first_features.lengths[index], second_features.lengths[reverse_edge]) &&
        close_angle(first_features.angles_deg[index], second_features.angles_deg[reverse_vertex]);
    }
