...
: Node("puzzle_solver_node")
{
  ...
  challenge_config.challenge_one_legacy_algorithm_enabled = declare_parameter<bool>(
    "challenge_one_legacy_algorithm_enabled", true);
  challenge_config.maximum_enumerated_edges = declare_parameter<int>(
    "challenge_one_maximum_enumerated_edges", 18);
  challenge_config.enumeration_worker_threads = declare_parameter<int>(
    "challenge_one_enumeration_worker_threads", 5);
  ...
  challenge_config.minimum_rectangularity = declare_parameter<double>(
    "challenge_minimum_rectangularity", config.minimum_rectangularity);
  ...
  // Task 3 first tries the specialized small-rectangle/exchangeable-piece
  // pattern path, then falls back to the task-2 geometry path when no
  // specialization is present.
  puzzle_solver::SolverConfig challenge_two_config = challenge_config;
  challenge_two_config.challenge_one_legacy_algorithm_enabled = false;
  challenge_two_config.minimum_rectangularity = declare_parameter<double>(
    "challenge_two_minimum_rectangularity", challenge_config.minimum_rectangularity);
  ...
  challenge_two_config.challenge_two_specialized_logic_enabled = declare_parameter<bool>(
    "challenge_two_specialized_logic_enabled", true);
  ...
  challenge_two_config.small_rectangle_min_rectangularity = declare_parameter<double>(
    "challenge_two_small_rectangle_min_rectangularity", 0.96);
  challenge_two_config.small_rectangle_min_aspect_ratio = declare_parameter<double>(
    "challenge_two_small_rectangle_min_aspect_ratio", 1.0);
  challenge_two_config.same_shape_relative_tolerance = declare_parameter<double>(
    "challenge_two_same_shape_relative_tolerance", 0.05);
  challenge_two_config.same_shape_angle_tolerance_deg = declare_parameter<double>(
    "challenge_two_same_shape_angle_tolerance_deg", 6.0);
  challenge_two_config.white_border_fast_solver_enabled = declare_parameter<bool>(
    "challenge_two_white_border_fast_solver_enabled", false);
  if (challenge_two_config.challenge_two_specialized_logic_enabled) {
    challenge_two_config.white_border_fast_solver_enabled = false;
  }
  ...
  const cv::Mat & rectified) const
{
  ...
  if (!rectified.empty() && rectified.type() == CV_8UC3) {
    debug = rectified.clone();
  } else {
    const int fallback_width = std::max(640, cvCeil(210.0 * pixels_per_mm_));
    const int fallback_height = std::max(900, cvCeil(297.0 * pixels_per_mm_));
    debug = cv::Mat(fallback_height, fallback_width, CV_8UC3, cv::Scalar(245, 245, 245));
    RCLCPP_WARN(
      get_logger(),
      "BUG_POINT:SOLVER_DEBUG_PLAN_FALLBACK rendering plan preview without rectified image");
  }
  ...
  const auto debug_message = to_bgr8_image(scene.header, debug);
