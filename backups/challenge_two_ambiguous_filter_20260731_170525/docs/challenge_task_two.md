...
[task,3]
  ...
  -> 沿每块轮廓边内侧采样白色度，识别扑克牌原始白边
  -> 白边直接锁定目标外框：白边不得参与内部拼接，最终外露边必须全部为白边
  -> 只在非白切割边之间做边长、端点、重叠和矩形闭合搜索
  -> 90--120 mm × 50--90 mm、总轮廓面积/矩形面积、每片外边和白边覆盖四边约束
  -> 完整目标轮廓位于 A4 下半区安全边界
  -> 若白边外框几何只有唯一拼图，直接采用
...
为准。当前任务三不再复用发挥题一的完整未知边枚举作为主路径，而是先用扑克牌白边
锁定外框，内部搜索只处理非白切割边，因此搜索速度应高于发挥题一。多解问题本身与
是否使用白边无关，发生在白边外框已经成立之后。
...
- `challenge_two_white_border_fast_solver_enabled=true` 时，白边是任务三外框硬约束：白边
  不进入内部拼接，最终外露边必须全部为白边并覆盖完整矩形四边。
...
- `WHITE_BORDER_EVIDENCE_REQUIRED`：至少一片碎片缺少可信白边，或总白边数量不足以锁定外框。
- `NO_FEASIBLE_PLACEMENT`：没有候选通过白边外框、几何和下半区放置硬约束。
