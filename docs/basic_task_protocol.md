...
当前串口默认配置为 `green_a4_normalized`。绿框左下角为 `(0,0)`，右下角为
`(1,0)`，左上角为 `(0,1)`，绿框中心为 `(0.5,0.5)`；X 向右为正，Y 向上为正，
按 A4 绿框内位置比例取值。A4 毫米坐标仍在内部保留为左上角原点、Y 向下，
下发前转换为 `x = a4_x_mm / 210`、`y = 1 - a4_y_mm / 297`。
...
- `x0,y0`：当前碎片中心在绿框内的归一化坐标。
- `x1,y1`：该碎片目标中心在绿框内的归一化坐标。
